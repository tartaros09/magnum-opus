...
\begin{figure}
    \scalebox{.6}{\begin{tikzpicture}[level distance=3em]
        \begin{scope}[yshift=-2cm]
            ...
            child {node{$u_{4}$}}
            child {node{$u_{5}$}};
            \node at ([xshift=3cm]root) {$\xrightarrow{\homomorphism}$};
        \end{scope}
...
Then $D_{\homomorphism}(\gamma) : \counting{5} \to 2^{\counting{7}}$ would be given by
...
%\begin{theorem}
%  \metaTypecheckingProblemFull\ can be solved in time $2^{\mathsf{polynomial}(\mathsf{input})}$, where the degree of the polynomial also depends on the input. 
%\end{theorem}
