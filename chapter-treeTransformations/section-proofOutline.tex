...
(eta)	edge (beta)
		...
		\node[right = 1cm of eta ] (arrow1)
                {$\xrightarrow{\phi_{\src}}$};
...
\draw[] ($(gammap)+(0,3mm)$)--(gammap) ;
		...
		\node at (gammap) [below left  = 3.5mm and 1mm of gammap] {\small $\phi(\gamma)$};
				...
				\node[right = 3.5cm of arrow1 ]  {$\xrightarrow{\sigma}$};
	...
	\node[fill=red!20, draw = red,  above right=5mm and 27mm of phanta,  regular polygon,regular polygon sides=3, text width = 4mm, label={[xshift=-6pt, yshift=-8mm]$\footnotesize\sigma(x)$}] (x1) {};
	
	\node[fill=red!20,draw = red, regular polygon,regular polygon sides=3, text width = 4mm, below left=8mm and 4mm of x1,   label={[xshift=-6pt, yshift=-8mm]$\footnotesize\sigma(x)$}] (x2) {};
	...
	\draw[dotted, thick] (x2) -- ($(x2)+(+2mm,-5mm)$) node[ right, xshift = 1pt] {\color{red}$q_{5}$}  ;
			...
			\node at (alphas) [below left  = 6.5mm and 2.7mm of alphas] {\scriptsize $\sigma(\phi(\alpha))$};
			\node at (betas) [below left  = 6.5mm and 2.7mm of betas] {\scriptsize $\sigma(\phi(\beta))$};
			\node at (gammas) [below left  = 6.5mm and 2.7mm of gammas] {\scriptsize $\sigma(\phi(\gamma))$};
	
					\node[above right =2.5cm and 13mm of phanta, text width =4cm]  (){Annotated Source tree };
	\end{tikzpicture}


	\Description{Simulation of the effect of running $A_{\sourceLang}$ on the source tree in the seed tree itself.}
...
One thing we ignored in the above explanation is that the variable $x$
